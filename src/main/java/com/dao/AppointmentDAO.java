...
public class AppointmentDAO {
	...
	public int countTodayAppointment() {
		int count = 0;
		try {
			String todayDate = LocalDate.now().toString();
			String sql = "select count(*) from appointment where appoint_date=?";
			PreparedStatement ps = conn.prepareStatement(sql);
	...
	public List<Appointment> getAllPendingAppointmentsSorted() {
		List<Appointment> list = new ArrayList<Appointment>();
		Appointment ap = null;
		
		try {
			String sql = "select * from appointment where status='Pending' order by appoint_date asc";
			PreparedStatement ps = conn.prepareStatement(sql);
	...
	public List<Appointment> getTodayPendingAppointments() {
		...
		try {
			String todayDate = LocalDate.now().toString();
			String sql = "select * from appointment where status='Pending' and appoint_date=? order by appoint_date asc";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, todayDate);
