...
public class DoctorDao {
	...
	public boolean registerDoctor(Doctor d) {
		...
		try {
			String sql = "insert into doctor(full_name, dob, qualification, specialist, email, mobno, password, photo) values(?,?,?,?,?,?,?,?)";
	...
	public boolean updateDoctor(Doctor d) {
		...
		try {
			String sql = "update doctor set full_name=?, dob=?, qualification=?, specialist=?, email=?, mobno=?, password=?, photo=? where id=?";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, d.getFullName());
			ps.setString(2, d.getDob());
			ps.setString(3, d.getQualification());
